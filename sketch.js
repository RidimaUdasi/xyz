...
function setup(){
  ...
  Background = createSprite(windowWidth/2,windowHeight/2,20,20)
  Background.addImage(backgroundImage)
  Background.scale = 1.2
  
  fruitNinja = createSprite(windowWidth/2,windowHeight/6,20,20)
  fruitNinja.addImage(fruitNinjaImage)
  fruitNinja.scale = 0.5
  
  //creating sword
  sword = createSprite(width+530,height+300,20,20);
  sword.addImage(swordImage)
  
  //To game over
  gameOver=createSprite(700,330,10,10);
  gameOver.addImage(gameOverImage);
  gameOver.scale=2;
  gameOver.visible = false;
  
  //To restart 
  restart=createSprite(700,440,10,10);
...
function spawnFruits() {
  if(World.frameCount%80 === 0) {
    var randomHeight = Math.round(random(windowHeight/10, windowHeight - 40))
    fruit = createSprite(width+600,randomHeight,20,20)
    fruit.scale = 0.2
    //fruit.debug = true
...
function spawnEnemies() {
  if(World.frameCount%200 === 0) {
    monster = createSprite(width+600,height+300,20,20)
